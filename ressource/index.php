...
if(isset($_POST['add'])){
	if(!empty($_POST['recc'])){
		...
		$hora_estimada = $hora_proyecto + $_POST['horas'];

		$add = $conn->prepare("UPDATE proyectos SET phorapro  = :hora   WHERE idp = :id ");
		$add->bindValue(':id', $idp);
		$add->bindValue(':hora', $hora_estimada  );
		$add->execute();
...
<select name="recc" class="form-control">
	<?php
		$recurso = $conn->query("SELECT id, rnombre FROM recurso");
		while ($row_recurso = $recurso->fetch()) {
	?>
			<option value="<?php echo $row_recurso['id'] ?>"><?php echo $row_recurso['rnombre'] ?></option>
	<?php
		}
	?>
</select>
