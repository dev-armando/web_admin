...
if(isset($_POST['add'])){
	...
	if(!empty($_POST['rnombre']) && !empty($_POST['rpass']) && !empty($_POST['rusuario']) ){
		...
		$add->bindValue(':nombre', strtolower($_POST['rusuario']));
		$add->bindValue(':pass', md5($_POST['rpass']));
		$add->bindValue(':cate', $_POST['cate']);
		$add->bindValue(':email', $_POST['rcorreo']);
		$add->bindValue(':fecha', $_POST['rfecha']);
		$add->execute(); 
		}
...
<p>
<span class="text-capitalize"><?=$fila['rnombre'];?></span>
 <span class="text-muted"><?=$fila['rusuario'];?> </span>
(<span class="text-muted"><?=$fila['rcorreo'];?></span>)
<a title="Editar"  <?= Helpers::addHrefID( 'editar.php', $fila['id']) ?> ><i class="far fa-edit"></i>
</a>
