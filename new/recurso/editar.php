...
if( isset( $_POST['edd'] )  ){
	...
	if( !empty($_POST['rpass']) ) {
		...
		$modelo->updateValorHora($rusuario , $_POST['rvhora']) ;
		...
		if($cate == '1' or $cate == '2' ){
			...
			$modelo2->update( array(
				'id'=>  $modelo2->getUsuarioID($rusuario),
				'status' => $status ,
				'pass'   => $rpass ,
				'cate'   => $cate ,
				'email'  => $rcorreo
...
<div class="form-group col-md-8">
<input readonly <?= Helpers::addValue($data['rusuario'])  ?>  type="text" name="rusuario" class="form-control" placeholder="Nombre de usuario (max 40 car)" maxlength="40">
</div>
<?php 
...
<div   class="form-group"><input type="password"   <?= Helpers::addValue($data['rpass'])  ?>  name="rpass" class="form-control" placeholder="Contrase&ntilde;a"></div>
